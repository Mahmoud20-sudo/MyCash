...
class ClientDetailsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.apply {
            ...
            val invoicesAdapter =
                InvoicesAdapter(requireContext(), invoicesCommunicator, viewModel.currency)
                ...
                when (response) {
                    is InvoiceState.SuccessShowInvoices -> {
                        if (response.data?.data?.isNotEmpty() == true) {
                            ...
                            binding.invoicesCounter.text =
                                "${getString(R.string.invoices)} (${response.data?.data?.size ?: 0})"
                        } else {
                            invoicesAdapter.submitList(response.data?.data)
                ...
                when (response) {
                    is ReceiptState.SuccessShowReceipts -> {
                        if (response.data?.data?.isNotEmpty() == true) {
                            ...
                            binding.receiptsCounter.text = getString(R.string.receipts)
                    ...
                    is ReceiptState.ServerError -> handleError(
                        response.error.getErrorMessage(
                            requireContext()
                        )
                    )
...
//            }
    ...
    private fun showPayReceiptDialog(receipt: ReceiptModel) {
        ...
        payReceiptDialog = PayReceiptDialog(
            requireContext(),
            receipt,
            viewModel.currency,
            parentFragmentManager,
            payReceiptCommunicator,
            currentReceiptModel
        )

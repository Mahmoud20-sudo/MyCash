    ...
    class ViewHolder(val binding: ItemInvoiceBinding) :
        ...
        fun bind(
            ...
            binding.apply {
                ...
                returnTextView.isVisible = item.isReturn == "1"
...
//                    SIMPLE_INVOICE.value -> Pair(if(item.type == "2") R.string.instant_invoice else R.string.simple_invoice, if(item.type == "2") R.drawable.ic_instant else 0)
//                    SALES_RETURNED.value -> Pair(R.string.sales_returned_invoices, R.drawable.ic_returned)
//                    else -> Pair(R.string.invoice, R.drawable.ic_receipt)
//                }

                val (title, drawableRes) = item.run {
                    val title = if(invoiceType?.toInt() == 2) R.string.tax_invoice else R.string.simplified_tax_invoice
...
//                    if(type?.toInt() == 2) Pair(title, R.drawable.ic_instant)
                ...
                item.isReturn.apply {
                    then { invoiceIv.setImageResource(R.drawable.ic_money_change) }
                    elze { invoiceIv.setImageResource(R.drawable.ic_receipt) }
                }
                ...
                invoicePaymentLayout.setOnClickListener {
                    if (item.paymentStatus == PAYMENT_NOT_COMPLETED.value.toString())
                        communicator.onUnpaidClick(item)
                }
            }
    ...
    interface Communicator {
        ...
        fun onUnpaidClick(invoice: InvoiceModel)
    }

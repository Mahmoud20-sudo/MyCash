...
class UsersFragment : Fragment() {
        ...
        viewModel.apply {
            ...
            getUsersResultState.collectOnLifecycle(lifecycleScope, viewLifecycleOwner) { response ->
                when (response) {
                    ...
                    is UsersState.ServerError -> handleError(
            ...
            deleteUsersResultState.collectOnLifecycle(
                ...
                when (response) {
                    ...
                    is UsersState.SuccessDeleteSingleUser -> response.data?.message?.let {
                        handleError(
                            it
                        )
                    }
    ...
    private val usersCommunicator = object : UsersAdapter.Communicator {
        ...
        override fun onClick(user: GetUsersDTO.Data.Data) {
            val action =
                UsersFragmentDirections.actionNavUsersFragmentToNavUserDetailsFragment(user)
            findNavController().navigate(action)
        }
    }

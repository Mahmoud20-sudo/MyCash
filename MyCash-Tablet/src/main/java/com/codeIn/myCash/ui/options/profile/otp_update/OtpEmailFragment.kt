...
import com.codeIn.myCash.databinding.FragmentOtpEmailBinding
...
class OtpEmailFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        oldEmail = OtpEmailFragmentArgs.fromBundle(
            requireArguments()
        ).email
        phone = OtpEmailFragmentArgs.fromBundle(
            requireArguments()
        ).phone
        email = OtpEmailFragmentArgs.fromBundle(
            requireArguments()
        ).newEmail
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.apply {
            ...
            resendOtpButton.setOnClickListener {
                ...
                viewModel.resendOtp(phone, oldEmail)
            }

            if (sequenceOf(phone, oldEmail).none { it.isNullOrBlank() }) {
                profileViewModel.editCode(phone!!,2,oldEmail!!)
            }

            confirmButton.setOnClickListener {
                binding.otpView.run {
                    if (this.otp.toString().isNotEmpty()) {
                        profileViewModel.emailUpdate(email!!, this.otp.toString().toInt())
                    } else
                        otpView.showError()
                }
            }
        ...
        profileViewModel.apply {
            emailResult.collectOnLifecycle(viewModelScope, viewLifecycleOwner) { response ->
                response?.data?.let {

...
class CreateAccountFragment : Fragment() {
    ...
    private fun handleInputError(invalidInput: InvalidInput) {
        when (invalidInput) {
            InvalidInput.EMPTY_EMAIL -> {
                binding.emailEditText.showError()
                CustomToaster.show(
                    context = requireContext(),
                    message = getString(R.string.empty_email_error_message),
                    isSuccess = false
                )
            }
            InvalidInput.EMPTY_PHONE -> {
                binding.phoneLayout.phoneNumberEditText.showError()
                CustomToaster.show(
                    context = requireContext(),
                    message = getString(R.string.phone_email_error_message),
                    isSuccess = false
                )
            }
            InvalidInput.EMPTY_PASSWORD -> {
                binding.passwordTextInputLayout.showError(binding.passwordEditText)
                CustomToaster.show(
                    context = requireContext(),
                    message = getString(R.string.empty_password_error_message),
                    isSuccess = false
                )
            }

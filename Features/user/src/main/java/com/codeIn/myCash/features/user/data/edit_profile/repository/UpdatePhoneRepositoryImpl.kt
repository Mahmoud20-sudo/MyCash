...
class UpdatePhoneRepositoryImpl @Inject constructor(
    private val api: ApiEditProfile,
    private val prefs: SharedPrefsModule,
    private val errorHandler: ErrorHandlerImpl
) : UpdatePhoneRepository {
    override suspend fun updatePhone(phone: String, code: Int): Response<UpdateProfileDTO> {
        return try {
            val lang = prefs.getValue(Constants.getLang())
            val token = prefs.getValue(Constants.getToken())
            val response = api.updatePhone(lang, token, phone, code)
            response

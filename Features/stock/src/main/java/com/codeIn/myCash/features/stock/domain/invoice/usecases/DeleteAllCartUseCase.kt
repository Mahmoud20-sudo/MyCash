...
class DeleteAllCartUseCase  @Inject constructor(private val cartRepository : CartRepository) {
    suspend operator fun invoke(productModel: ProductModel, productsInCart: ProductsInCart?) : ProductsInCart? {
        return cartRepository.deleteAllFromCart(productModel , productsInCart)
    }
}
...
package com.codeIn.myCash.ui.home.clients_vendors.memorandum
...
import com.codeIn.myCash.ui.home.invoices.memorandum.MemorandumViewModel
import com.codeIn.myCash.ui.home.invoices.memorandum.ProductInInvoiceAdapter
...
@AndroidEntryPoint
class ClientMemorandumFragment : Fragment() {
...
//                    is MemorandumState.ServerError -> handleError(
//                        response.error.getErrorMessage(
//                            requireContext()
//                        )
//                    )
    ...
    private fun FragmentPaymentSummaryBinding.bindSummaryInvoice(memorandumModel: MemorandumModel?) {
        ...
        addedTaxView.text =
            "${getString(R.string.added_tax)} (${memorandumModel?.invoiceModel?._tax}%)"
        addedTaxSummaryView.text =
            "${getString(R.string.added_tax)} (${memorandumModel?.invoiceModel?._tax}%)"
    ...
    private fun FragmentPaymentSummaryBinding.bindInfo(memorandumModel: MemorandumModel?) {
        ...
        taxNumber.text =
            memorandumModel?.invoiceModel?.cashierModel?.accountInfo?.taxRecord ?: "----"
        deviceNumber.text =
            memorandumModel?.invoiceModel?.cashierModel?.subscription?.device?.device?.name
                ?: "----"
        nameStore.text =
            memorandumModel?.invoiceModel?.cashierModel?.accountInfo?.commercialRecordName
        ...
        if (!memorandumModel?.invoiceModel?.qrZatca.isNullOrEmpty()) {
            val bitmapZatcaQr =
                QR.generateQrCode(memorandumModel?.invoiceModel?.qrZatca ?: "", requireContext())

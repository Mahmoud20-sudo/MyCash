...
import com.codeIn.myCash.ui.home.products.add_new_product.NewProductViewModel
import com.google.android.material.snackbar.Snackbar


class AddCategoriesAdapter() : RecyclerView.Adapter<AddCategoriesAdapter.ViewHolder>(){
    private lateinit var dataList: LiveData<ArrayList<CategoryModel>>
    private var context: Context? = null
    private var viewModel : NewProductViewModel? = null
    private lateinit var lifecycleOwner: LifecycleOwner
    private lateinit var listener : Listener

    constructor(dataList: LiveData<ArrayList<CategoryModel>>,
                context: Context?, viewModel: NewProductViewModel,
    ...
    class ViewHolder(private var binding: ItemCategoryAddProductBinding,
                     var dataList: LiveData<ArrayList<CategoryModel>>, val viewModel: NewProductViewModel,
        ...
        fun bind(item: CategoryModel ) {
            binding.apply {
                ...
                if (item.addChild){
                    ...
                    if (adapterPosition > 0)
                    {
                        if (viewModel.selectedCategories.value!![adapterPosition - 1].id != 0)
                            ...
                            viewModel.categoriesAutoCompleteSearchInAdapter.value = ArrayList<AutoCompleteModelSearch>()
                    }
                    else if (adapterPosition == 0 ){
                        viewModel.getCategories(null)

                        category.setOnFocusChangeListener { _, b ->
                ...
                addCategory.setOnClickListener {
                    if (!item.addChild && !item.name.isNullOrEmpty())
                    {
                        item.addChild = true
                        ...
                        Snackbar.make(binding.root, binding.root.context.getString(R.string.empty_category), Snackbar.LENGTH_SHORT).show()
                    }
                }
                category.addTextChangedListener(object : TextWatcher {
                    override fun beforeTextChanged(charSequence: CharSequence, i: Int, i2: Int, i3: Int) {}

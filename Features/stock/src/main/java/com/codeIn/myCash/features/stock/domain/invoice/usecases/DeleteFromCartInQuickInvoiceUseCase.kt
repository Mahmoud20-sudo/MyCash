...
class DeleteFromCartInQuickInvoiceUseCase @Inject constructor(private val cartRepository : CartInQuickInvoiceRepository) {
    suspend operator fun invoke(productModel: ProductInQuickInvoice, productsInCart: ProductsInCartInQuickInvoice?) : ProductsInCartInQuickInvoice?{
        return cartRepository.deleteFromCart(productModel , productsInCart)
    }
}
...
//                    is MemorandumState.ServerError -> handleError(response.error.getErrorMessage(requireContext()))
    ...
    private fun FragmentPaymentSummaryBinding.bindSummaryInvoice(memorandumModel: MemorandumModel?){
        ...
        memorandumModel?.invoiceModel?.invoiceNumber?.let {
            originalInvoiceContainer.visible()
            originalInvoiceSummaryContainer.visible()
            originalInvoiceNumber.text = "#${it}"
            originalInvoiceSummary.text = "#${it}"
        }

        when(memorandumModel?.paymentType){
            PaymentType.CASH.value.toString() -> {
                paymentMethod.text = getString(R.string.invoice_cash)
...
//            .into(logoStore)
        ...
        clientInfoContainer.isVisible = memorandumModel?.invoiceModel?.client != null

...
class LogInFragment : Fragment() {
    ...
    @Inject
    lateinit var settingsLanguage: MyCashPreferences
    @Inject
    lateinit var settings: SharedPrefsModule
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.apply {
            freeTryButton.setOnClickListener {
                navigateTo(
                    com.codeIn.myCash.ui.authentication.log_in.log_in.LogInFragmentDirections.actionNavigationLogInFragmentToNavigationCreateAccountFragment(
            ...
            forgotPasswordTextView.setOnClickListener {
                navigateTo(com.codeIn.myCash.ui.authentication.log_in.log_in.LogInFragmentDirections.actionNavigationLogInFragmentToNavigationForgotPasswordFragment())
            }
            ...
            rememberMe.setOnClickListener {
                if (rememberMe.isChecked) {
                    ...
                    settings.putValue(Constants.isRememberMe(), "0")
                    settings.putValue(Constants.getPhone(), "")
                    settings.putValue(Constants.getPassword(), "")
                }

            }
            if (settings.getValue(Constants.isRememberMe()).equals("1")) {
                phoneLayout.phoneNumberEditText.setText(
                    settings.getValue(Constants.getPhone())
                )
                passwordEditText.setText(
                    settings.getValue(Constants.getPassword())
                )
            ...
            passwordEditText.addTextChangedListener(object : TextWatcher {
                ...
                override fun afterTextChanged(s: Editable) {
                    if (settings.getValue(Constants.isRememberMe()).equals("1")) {
                        ...
                        settings.putValue(Constants.getPassword(), s.toString())
                    }
                }
            })
                ...
                when (response) {
                    ...
                    is Success -> {
                        if (response.data?.status == "1"){
                            ...
                            navigateTo(
                                com.codeIn.myCash.ui.authentication.log_in.log_in.LogInFragmentDirections.actionNavigationLogInFragmentToOtpVerificationSignUpFragment(
                                    response.data?.phone?:""
                                )
                            )
                        }
    ...
    private fun setLanguagesViews() {
        ...
        val lang = settingsLanguage.getString(MyCashPreferencesKeys.LANG).ifEmpty {
            settingsLanguage.getString(MyCashPreferencesKeys.LANG, Locale.getDefault().language)
        }

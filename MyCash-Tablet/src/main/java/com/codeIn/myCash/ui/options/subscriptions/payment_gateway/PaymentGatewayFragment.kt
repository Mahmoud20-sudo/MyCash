package com.codeIn.myCash.ui.options.subscriptions.payment_gateway
...
import com.codeIn.myCash.ui.authentication.sign_up.payment_gateway.PaymentGatewayViewModel
...
class PaymentGatewayFragment : Fragment()  {
    ...
    private val args: com.codeIn.myCash.ui.options.subscriptions.payment_gateway.PaymentGatewayFragmentArgs by navArgs()
            ...
            payResult.collectOnLifecycle(viewModelScope, this@PaymentGatewayFragment) { response ->
                when (response) {
                    ...
                    is GeneralResponse.ResponseError -> handleError(
                        response.message ?: getString(R.string.error)
                    )
        ...
        binding.webView.webViewClient = object : WebViewClient() {
            override fun shouldOverrideUrlLoading(view: WebView?, url: String?): Boolean {
                ...
                if (link.contains(AuthConsts.PAYMENT_SUCCESS_LINK)) {
                    (requireActivity() as MainActivity).viewModel.expire.value = false

...
class DeleteFromCartInReturnInvoiceUseCase  @Inject constructor(private val cartInReturnInvoiceRepository: CartInReturnInvoiceRepository) {
    suspend operator fun invoke(productModel: ProductInInvoiceModel?, productInReturnInvoice: ProductInReturnInvoice?) : ProductInReturnInvoice? {
        return cartInReturnInvoiceRepository.deleteFromInvoice(productModel , productInReturnInvoice)
    }
}
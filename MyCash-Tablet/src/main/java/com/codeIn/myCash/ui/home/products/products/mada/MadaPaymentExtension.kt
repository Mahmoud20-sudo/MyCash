    ...
    val nearPayListener = object : NearPayPayment.Companion.Listener {
        override fun onPurchaseProcess(
            ...
            if (code == 0){
                ...
                productsViewModel.createMadaTransaction(amount , transactionData?.receipts?.get(0)?.transaction_uuid  ,
                    transactionData?.receipts?.get(0)?.approval_code?.value.toString(),
                    transactionData?.receipts?.get(0)?.created_at.toString() , "1")
            }
            else
            {
                when (code) {
    ...
    val nearPayListener = object : NearPayPayment.Companion.Listener {
        ...
        override fun onRefundProcess(
            ...
            if (code == 0){
                ...
                productsViewModel.createMadaTransaction(amount , transaction?.receipts?.get(0)?.transaction_uuid  ,
                    transaction?.receipts?.get(0)?.approval_code?.value.toString(),
                    transaction?.receipts?.get(0)?.created_at.toString() , "2")
            }
